...
const PaymentInstructionsScreen = ({ route, navigation }) => {
  ...
  const sharePaymentInfo = async () => {
    try {
      const message = `
💰 Información de Pago CDE Inversiones

Monto: RD$ ${amount.toLocaleString()}
Banco: ${bank?.name || paymentMethod?.account_info?.bank_name}
Titular: ${paymentMethod?.account_info?.account_holder}
Cuenta: ${paymentMethod?.account_info?.account_number}
Tipo: ${paymentMethod?.account_info?.account_type}
        ...
        <View style={styles.section}>
          ...
          <View style={styles.referenceCard}>
            <View style={styles.referenceIcon}>
              <Ionicons name="barcode-outline" size={32} color="#3182ce" />
            </View>
            <View style={styles.referenceContent}>
              <Text style={styles.referenceLabel}>Use esta referencia en su transferencia</Text>
